...
def create_hvac(hvac_data):
    if not 'area' in hvac_data or not isinstance(hvac_data['area'], str):
...
def update_hvac(uuid, hvac_data):
    ...
    if 'update_period' in hvac_data  and isinstance(hvac_data['update_period'], int):
    ...
    try:
        if event['httpMethod'] == "GET":
            ...
            if event['body'] and not event['pathParameters']:
                ...
                raise Exception('Error. HTTP body required for POST to create hvac.')

        elif event['httpMethod'] == "PUT":
            if event['body'] and event['pathParameters'] and 'uuid' in event['pathParameters']:
                return update_hvac(event['pathParameters']['uuid'], json.loads(event['body']))
